...
export default function Hero({ className = "" }: HeroProps) {
  ...
  const handlePrevious = () => {
    const newIndex =
      currentIndex > 0 ? currentIndex - 1 : heroEvents.length - 1;
    scrollToIndex(newIndex);
  };

  const handleNext = () => {
    const newIndex =
      currentIndex < heroEvents.length - 1 ? currentIndex + 1 : 0;
    scrollToIndex(newIndex);
  };
  ...
  useEffect(() => {
    // Don't auto-scroll if paused or if there are no events
    if (isPaused || heroEvents.length === 0) {
    ...
    // Set up interval to auto-scroll every 4 seconds
    autoScrollIntervalRef.current = setInterval(() => {
      const nextIndex =
        currentIndexRef.current < heroEvents.length - 1
  ...
  }, [isPaused, scrollToIndex, heroEvents.length]);
