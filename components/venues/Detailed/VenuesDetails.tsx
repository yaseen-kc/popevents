/**
 * Venue detail component - displays venue image and description
 *
 * @component
 * @example
 * ```tsx
 * <VenuesDetails name="bahrain-national-theatre" />
 * ```
 */
...
import { getVenueDetailById } from "@/constants/entities/venues";
...
interface VenuesDetailsProps {
  /** Venue ID/slug to display details for */
  name: string;
}
...
/**
 * Venue details component displaying venue image and description
 *
 * @param props - Component props
 * @returns Venue details section element
 */
export default function VenuesDetails({ name }: VenuesDetailsProps) {
  const { language } = useTranslation();
  const [venueDetail, setVenueDetail] = useState<VenueDetail | null>(null);

  useEffect(() => {
    async function loadVenueDetail() {
      const detail = await getVenueDetailById(name, language);
  ...
  const { name: venueName, description, image, location, events } = venueDetail;
      ...
      <section className="flex w-full justify-center bg-white px-4 py-16 md:px-6 md:py-20 lg:py-24">
        <div className="flex w-full max-w-[984px] flex-col items-start gap-[65px] md:flex-row lg:max-w-[1244px]">
          ...
          <div className="flex w-full flex-col items-start gap-[26px] pt-[14px] md:w-[389px] md:flex-none lg:w-[647px]">
            ...
            {/* Location */}
            <p className="font-be-vietnam-pro text-base font-medium leading-6 text-[#8B8123]">
              {location}
            </p>
